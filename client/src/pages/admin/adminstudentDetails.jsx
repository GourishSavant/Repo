...
import { fetchStudents } from "../../redux/slices/getStudentsSlice.jsx"; // Redux action to fetch students
const AdminStudentDetails = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  // Redux state
  const studentsData = useSelector((state) => state.getStudents.students);
  const loading = useSelector((state) => state.getStudents.loading);
  const error = useSelector((state) => state.getStudents.error);
  const [filteredData, setFilteredData] = useState([]);
  const [classOption, setClassOption] = useState("");
  const [sectionOption, setSectionOption] = useState("");
  const [keyword, setKeyword] = useState("");
  ...
  const [currentPage, setCurrentPage] = useState(1);
  const [additionalKeyword, setAdditionalKeyword] = useState(""); // New state for additional search box
  const itemsPerPage = studentsData.length;
  useEffect(() => {
    // Dispatch action to fetch students on mount
    dispatch(fetchStudents());
  }, [dispatch]);
  useEffect(() => {
    // Update filteredData whenever studentsData changes
    if (studentsData) setFilteredData(studentsData);
  }, [studentsData]);
  // State for Column Visibility
    const [visibleColumns, setVisibleColumns] = useState({
      admissionNo: true,
      ...
      if (keyword) {
        filtered = filtered.filter(
          (student) =>
            ...
            student.roll_no.includes(keyword)
        );
      }
      setFilteredData(filtered);
      setCurrentPage(1);
...
{viewType === "list" && (
                  ...
                  {currentItems.length > 0 ? (
                    currentItems.map((student) => (
                      <tr key={student.admission_no}>
                        <td className="border p-2">{student.admission_no}</td>
                      ...
                      <button
              className="text-blue-500 hover:text-blue-600"
              onClick={() => navigate(`/admin/profile/${student.student_id}`)}
            >
              <FontAwesomeIcon icon={faList} />
            </button>
                        {/* Edit Icon */}
                        <button
                          className="text-green-500 hover:text-green-600"
                          onClick={() => navigate(`/admin/student/${student.student_id}/edit`)}                        >
                          <FontAwesomeIcon icon={faEdit} />
                        </button>
                        {/* $ Icon */}
                        <button
                          className="text-yellow-500 hover:text-yellow-600"
                          onClick={() => navigate(`/admin/student/${student.student_id}/fees`)}                        >
                          <FontAwesomeIcon icon={faDollarSign} />
                        </button>
                      </div>
                    </td>
      ...
      <span className="text-4xl text-gray-600"></span>
                ...
                <div className="w-4/5 flex flex-wrap">
                  <div className="w-full md:w-1/2 mb-2">
                    <h3 className="text-lg font-semibold text-blue-500">{student.firstname} {student.lastname}</h3>
                    <p className="text-sm">Class: {student.class}({student.section})</p>
                    <p className="text-sm">Admission No: {student.admission_no}</p>
                    <p className="text-sm">Date Of Birth: {student.dob}</p>
                    <p className="text-sm">Gender: {student.gender}</p>
                  </div>
                  <div className="w-full md:w-1/2 mb-2">
                    <p className="text-sm">
                      <strong>Local Identification Number:</strong> {student.local_identification_number || "N/A"}
                    </p>
                    <p className="text-sm">
                      <strong>Guardian Name:</strong> {student.guardian_name}
                    </p>
                    <p className="text-sm">
                      <strong>Guardian Phone:</strong> :telephone_receiver: {student.guardian_phone}
                    </p>
                    <p className="text-sm">
